...
pub async fn streak(ctx: Ctx<'_>) -> R {
  ...
  let msg = if diff < MIN_TRESH {
    format!(
      "Your streak is {streak} and you need to wait {} to continue your streak",
      nicefmt(MIN_TRESH - diff)
    )
  } else if diff >= MIN_TRESH && diff < MAX_TRESH {
    format!(
      "Your streak is {streak} and you have {} left to continue your streak",
      nicefmt(MAX_TRESH - diff)
    )
  } else {
    if prev_ts == 0 {
      ...
      format!(
        "Your streak of {streak} expired {} ago",
        nicefmt(diff - MAX_TRESH)
      )
    }
  };
  ctx
    .send(|m| m.ephemeral(true).reply(true).content(msg))
    .await?;
  Ok(())
}
